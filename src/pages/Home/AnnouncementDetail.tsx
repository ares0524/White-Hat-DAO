...
import { allAnnouncements } from "./announcements";
import { allAnnouncementTitles } from "./announcements";
import { allAnnouncementTags } from "./announcements";
...
export const AnnouncementDetail = () => {
    ...
    const [announcementTags, setAnnouncementTags ] = useState<string[]>([]);
    // const makeComment = () => {
    ...
    // }
    useEffect(() => {
        const announceId = Number(window.localStorage.getItem('announcement'));
        announceId!=null? setAnnouncement(allAnnouncements[announceId]): setAnnouncement('');
        announceId!=null? setAnnouncementTitle(allAnnouncementTitles[announceId]): setAnnouncementTitle('');
        announceId!=null? setAnnouncementTags(allAnnouncementTags[announceId]): setAnnouncementTags([]);
    }, [])
                    ...
                    <Box sx={BoxStyle} padding={2}>
                        ...
                        <Box color={"#404040"} marginTop={2}>
                            ...
                            <Box display={'flex'} gap={1}>
                                {
                                    announcementTags.map((tag, key) => {
                                        return (
                                            <Typography key={key} sx={TagStyle} style={{backgroundColor:'white'}}>
                                                {tag}
                                            </Typography>
                                        )
                                    })
